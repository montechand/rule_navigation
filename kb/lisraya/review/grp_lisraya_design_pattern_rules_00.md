...
- rule_text: Email body width is fixed at 600px, centered; sections must render edge-to-edge at exactly 600px (section.width) so assembled sections butt cleanly. Mobile breakpoint is 480px (layout.mobile_breakpoint); all multi-column layouts collapse to single column.
- intent: Guarantee seamless zero-gap section assembly and predictable mobile reflow.

### rule_lisraya_internal_section_grid_options
- class=layout scope=org_baseline hardness=strong_default polarity=should sections=None constraint=binding
- rule_text: Internal section grid follows a 12-column mental model; practical layouts are 1-col, 2-col (50/50), or image/text split (≈40/60, layout.grid.image_text_split).
- intent: Constrain layouts to a small set of predictable grid patterns.
...
- rule_text: Section side padding is 24px left/right on ALL sections (section.padding.sides) — this consistency is what makes assembly seamless; full-bleed imagery/gradient backgrounds may run to 600px but text within them still respects the 24px insets. Vertical rhythm uses an 8px base unit (layout.base_unit); all padding/margins are multiples of 8. Each section owns its own 32px top AND bottom internal padding (section.padding.vertical) with no external margins between sections — assembly is zero-gap stacking. Heading→body gap 16px; paragraph→paragraph 16px; body→callout/CTA 24px (layout.gap.body_callout); image→caption 8px (layout.gap.image_caption).
- intent: Enforce a consistent 8px-based spacing system enabling zero-gap section stacking.
...
- rule_text: Primary CTA buttons use Brand Blue #00529b background (cta.button.fill) with White #ffffff text (cta.button.label.color), Nunito Sans Bold 16px, padding 14px 32px, minimum 48px touch-target height (brand accessibility rule: ≥48px clickable areas), and a 24px fully-rounded pill corner radius echoing the rounded brand aesthetic. Secondary CTA uses a White background, 2px Brand Blue border (cta.button.secondary.border) and Brand Blue text, with the same metrics.
- intent: Lock CTA button appearance and touch-target accessibility across all sections.
...
- intent: Preserve a single clear call-to-action focus within each section.
