...
- rule_text: The campaign headline 'YOU DESERVE MORE' is set in ALL CAPS, always.
- intent: Locked campaign lockup treatment.
...
- rule_text: Chart/infographic headlines are set in ALL CAPS, in Agenda (print) / Nunito Sans Bold or heavier (email fallback).
- intent: Distinct, consistent styling for chart headers.

### rule_lisraya_body_sentence_case_no_all_caps
- class=typography scope=org_baseline hardness=strong_default polarity=must_not sections=None constraint=binding
- rule_text: Body copy is sentence case. Do not use ALL CAPS for body copy, buttons, or subheads other than the campaign headline and chart/infographic headline cases.
- intent: Reserve ALL CAPS for the sanctioned exceptions only.
