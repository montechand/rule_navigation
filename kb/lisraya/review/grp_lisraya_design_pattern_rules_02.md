...
### rule_lisraya_patient_lifestyle_photography_treatment
...
- intent: Ground imagery in relatable, aspirational patient life moments.

### rule_lisraya_images_housed_in_accent_shape_crop
- class=imagery scope=brand hardness=hard_constraint polarity=must sections=None constraint=binding
- rule_text: Images must be housed in the brand Accent shape (§2.5) — asymmetric rounded-corner crops — not plain rectangles or circles. Waves and gradients may overlay photography.
- intent: Maintain the signature asymmetric accent-shape framing for all photography.
