...
- class=assembly scope=brand hardness=strong_default polarity=must sections=None constraint=ordering
- rule_text: DTP emails alternate section backgrounds in a fixed rhythm: purple hero image → white content section (efficacy + side-effect bullets) → white or light photo section (patient story / lifestyle) → green accent panel (#01A47E, affordability or symptom-icon trio) → purple campaign image section ('BECAUSE IBSRELA CAN' lockup) → locked End Matter.
- intent: Keep DTP email section backgrounds in a recognizable, consistent brand rhythm.

### rule_ibsrela_sections_butt_edge_to_edge_no_dividers
...
- intent: Achieve clean section transitions through color/full-bleed changes rather than added dividers or margins.

### rule_ibsrela_green_panel_at_most_once_reserved
- class=color_application scope=brand hardness=strong_default polarity=must_not sections=['affordability', 'symptom_trio'] constraint=exclusivity
...
- intent: Preserve the green accent as a rare, single-purpose emphasis panel per email.
