...
### rule_ibsrela_dtp_cta_label_format
- class=cta scope=brand hardness=strong_default polarity=must sections=['cta'] constraint=cardinality
- rule_text: Approved DTP CTA labels are short verb phrases in sentence case (not ALL CAPS), 1–3 words. Approved examples: "Explore IBSRELA", "Watch Regina's story", "Watch video", "Learn more about how IBSRELA is different", "Yes, I have".
- intent: Keep DTP CTA labels short, action-oriented, and consistently cased.
...
- intent: Allow poll/segmentation interactions styled as a primary CTA.

### rule_ibsrela_dtp_no_hcp_get_connected_label
- class=cta scope=brand hardness=hard_constraint polarity=must_not sections=['cta'] constraint=exclusivity
- rule_text: DTP CTAs must not use the HCP library default "Get connected"; that label is HCP-scope only.
- intent: Prevent HCP-scope labels from appearing in patient-facing CTAs.

### rule_ibsrela_dtp_cta_matches_section_topic
- class=cta scope=brand hardness=strong_default polarity=must sections=['cta'] constraint=None
- rule_text: Per-section CTAs must match the section's topic: a patient-story section → "Watch [name]'s story"; a product overview section → "Explore IBSRELA" or "Learn more about how IBSRELA is different".
- intent: Ensure CTA relevance to the section it accompanies.
