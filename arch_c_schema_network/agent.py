...
5. Call finalize_section_ruleset with your section_type_mapping and an `excluded` reason
   for every candidate you drop; per-rule rationale = the best shard evidence. If the
   coverage check rejects the call listing unaccounted rules, resolve each — include it
   or exclude it with a reason — and retry.
"""
